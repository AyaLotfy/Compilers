...
void Expression::infixToPostfix() {
	...
	for (int i = 0; i < l; i++) {
		...
		std::cout << "loop i = " << i << "size = " << NFS.size() << endl;

		if (s[i] == ' ') {
			continue;
		}
		// If the scanned character is an ï¿½(ï¿½, push it to the stack.
		if (s[i] == '(') {
			st.push(",");
			st.push("(");
		}
		// If the scanned character is an ï¿½)ï¿½, pop and to output string from the stack
		// until an ï¿½(ï¿½ is encountered.
		else if (s[i] == ')') {
...
std::vector<int> Expression::addOperand(std::string operand) {
...
std::vector<int> Expression::createSM(std::string operand) {
...
std::vector<int> Expression::addOperand(int index) {
...
std::vector<int> Expression::createVector(int a, int b) {
	std::vector<int> v;
	v.push_back(a);
	v.push_back(b);
	return v;
...
vector<int> Expression::eval(int op1, int op2, int op3, int op4,
		vector<int> operate) {

	std::cout << "evaluation " << op1 << op2 << op3 << endl;
...
vector<int> Expression::evalPostfix() {
	stack<vector<int>> s;
	vector<int> ch;
	vector<int> val;
...
vector<int> Expression::OR(int op1, int op2, int op3, int op4) {
...
//create end state
	...
	NFS[op2].push_back(createVector(-1, stateNum + 1));
	NFS[op4].push_back(createVector(-1, stateNum + 1));
...
vector<int> Expression::AND(int op1, int op2, int op3, int op4) {
...
//link
	NFS[op4].push_back(createVector(-1, op1));

	return startEnd;
}

vector<int> Expression::DASH(int op1, int op2, int op3, int op4) {

	int startSym = NFS[op3][0][0];
	int endSym = NFS[op1][0][0];
...
vector<int> Expression::Closure(int op1, int op2, int op3, int op4) {
...
vector<int> Expression::PClosure(int op1, int op2, int op3, int op4) {
